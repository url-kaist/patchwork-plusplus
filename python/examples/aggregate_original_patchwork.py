...
def aggregate_seq(path: str, protocol: str = "patchwork") -> dict:
    ...
    if protocol == "patchworkpp":
        if rows.shape[1] < 8:
            raise ValueError(f"{path} only has {rows.shape[1]} columns; rerun patchwork "
...
def main():
    ...
    ap.add_argument("--protocol", choices=["patchwork", "patchworkpp"], default="patchwork")
    args = ap.parse_args()
    ...
    with open(args.output_csv, "w", newline="") as fp:
        w = csv.writer(fp)
        w.writerow(["seq", "num_frames", "precision", "recall", "f1",
                    "precision_naive", "recall_naive", "f1_naive"])
        for name, m in rows:
            w.writerow([name, m["num_frames"],
                        f"{m['precision']:.4f}", f"{m['recall']:.4f}", f"{m['f1']:.4f}",
                        f"{m['precision_naive']:.4f}", f"{m['recall_naive']:.4f}",
                        f"{m['f1_naive']:.4f}"])
    ...
    print("-" * len(header))
    for name, m in rows:
        print(f"{name:>5} | {m['num_frames']:>6d} | "
